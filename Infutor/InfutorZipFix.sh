#!/bin/bash
#splits files into smaller chunks, sets them to lower case, runs batches of 5 to 10 every time period.
#created and in use since 2019-09-04  -- Dave Babler
rm -f explodedFiles.txt
rm -f cURL_log.txt
rm -f scrubtimes.txt
FRONT="curl  --insecure -X POST \
 https://104.152.217.27/api/services/CheckFTPFile \
...
printf "This program was written by Dave Babler d a b a b l e r (a) outlook\n"
printf '%s\n' "-----------------------------------------------------------"
...
do
	FULLZIP="$VAR_FRONT$i$VAR_ZIP_END"
	#remember switches go immediately after the program call in a script
	unzip -q $FULLZIP  
	#statements
...
printf "     -- Local file processing has been completed --        \n"
printf "___________________________________________________________\n"
printf "___________________________________________________________\n"
printf "___________________________________________________________\n"
printf "___________________________________________________________\n"
...
for (( i = $startNumber; i < $loopStopper; i++ )); do
	...
	echo "$SOMETHING began scrub $now" >> scrubtimes.txt
		...
		if [ "$remainder" -eq "0" ]
		 then 
		 	printf "I am going to rest for 90 minutes"
			 sleep 90m
			echo "done sleeping!"
		fi
done
